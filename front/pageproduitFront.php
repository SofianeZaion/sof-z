...
<div class="container py-2">
    ...
    if (!is_null($categoryId)) {
        $sqlState = $pdo->prepare("SELECT * FROM produit WHERE id_categorie=? ORDER BY date_creation DESC"); // Utilisation de la colonne 'date_creation'
        $sqlState->execute([$categoryId]);
        $produits = $sqlState->fetchAll(PDO::FETCH_OBJ);
    } else {
        $produits = $pdo->query("SELECT * FROM produit ORDER BY date_creation DESC")->fetchAll(PDO::FETCH_OBJ); // Utilisation de la colonne 'date_creation'
    }
                    ...
                    foreach ($categories as $categorie) {
                        ...
                        <li class="list-group-item <?= $active ?>">
                            <a class="btn btn-default w-100"
                               href="indexFront.php?id=<?php echo $categorie->id ?>">
