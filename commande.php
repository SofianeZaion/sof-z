...
INNER JOIN utilisateur ON commande.id_client = utilisateur.id 
                                   WHERE commande.id = ?
                                   ORDER BY commande.date_achat DESC');    //permettant de voir sous la bdd la date des achats effectués
...
<div class="container py-2">
    ...
    <table class="table table-striped table-hover">
        ...
        <tr>
            ...
            <th>Total</th>
            <th>Date</th>
            <th>Date_achat</th>   //ajout de date_achat dans le details des commandes
            <th>Opérations</th>
        </tr>
        </thead>
        <tbody>
        ...
        <tr>
            ...
            <td><?php echo $commande['total'] ?> <i class="fa fa-euro-sign"></i></td>
            <td><?php echo $commande['date_creation'] ?></td>
            <td><?php echo $commande['date_achat'] ?></td>  // affiche directement la colonne date_achat sur le recapitulatif de la commande
            <td>
    ...
    <table class="table table-striped table-hover">
        ...
        <?php foreach ($lignesCommandes as $lignesCommande) : ?>
            <tr>
                ...
                <td><?php echo $lignesCommande->prix ?> <i class="fa fa-euro-sign"></i></td>
                <td>x <?php echo $lignesCommande->quantite ?></td>
                <td><?php echo $lignesCommande->total ?> <i class="fa fa-euro-sign"></i></td>
            </tr>
        <?php endforeach; ?>
        </tbody>
    </table>
