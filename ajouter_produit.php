...
<div class="container py-2">
    ...
    if (isset($_POST['ajouter'])) {
        ...
        $date = date('Y-m-d');
        $dateAchat = $_POST['DateAchat']; // Correction du nom du champ
        ...
        if (!empty($libelle) && !empty($prix) && !empty($categorie)) {
            // Correction du nombre de variables liées dans la requête préparée
    ...
    <form method="post" enctype="multipart/form-data">
        ...
        <label class="form-label">Remise: <output name="discountOutput">0%</output></label>
        <input type="range" value="0" class="form-control" name="discount" min="0" max="90" oninput="discountOutput.value = this.value + '%'">
